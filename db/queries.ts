 ...
 *
 * Strength blocks only. Twenty-one thrusters at 43kg in a Fran is prescribed
 * volume, not an attempt at a 21-rep max — counting it would put a rep max on
 * this screen that the records screen refuses to show, and the two would
 * disagree. candidatesForBlock() applies the same rule to the records cache.
 */
...
function candidatesForBlock(
  ...
  // A load record can only come from strength work. A WOD's one
